...
Inside the live desk you can:

- paste a Pons V2 token and start or stop monitoring without a second terminal
- see live RPC health, latest block, sweep count, last sweep, and next sweep
- track deployer share, curve reserve or pool price, trades/swaps, fees, phase, and deployer history
- watch history grow every sweep, including flat-state samples when nothing changes
- read a per-sweep live feed plus the persistent deterministic signal timeline
...
- `.canary/watchlist.json` - tokens the board resumes after restart

The board is intentionally local-only and read-only. It creates no wallet client, imports no account, stores no private key, and has no transaction path.
